...
def filter_fisheries():
    # Using the checkboxes on the main fisheries page
...
def profile(user):
    ...
    if request.method == "POST":
        ...
        # If a user changes items in the profile
        # capture this and update the profile
        updated_profile = {"$set": {
            "email": request.form.get("email").lower(),
            "fname": request.form.get("fname").lower(),
            "lname": request.form.get("lname").lower(),
    ...
    sorted_catches = sorted(
        catches, key=lambda catch: catch["weight"], reverse=True)
    total_fish_weight = 0
    if len(catches) == 0:
        average_fish_weight = 0
        return render_template(
            "profile.html", account=account, catches=catches,
        ...
        # Make sure the person executing this
        # function is a session user
        if session["user"]:
            return render_template(
                "profile.html", account=account, catches=catches,
                total_fish_weight=total_fish_weight,
                average_fish_weight=average_fish_weight,
                sorted_catches=sorted_catches)
...
def logout():
    ...
    # # all sessions cookies are removed
    session.clear()
...
def add_fishery():
    # Make sure the person executing this
    # function is an admin
    if "is_admin" in mongo.db.accounts.find_one(
        ...
        if request.method == "POST":
            ...
            if (request.form.get("syndicate_daynight") == ""):
                ...
                syndicate_daynight = float(request.form.get(
                    "syndicate_daynight"))
            ...
            fishery_facilities = {
                ...
                "onsite_tackle_shop": bool(request.form.get(
                    "onsite_tackle_shop")),
                ...
                "takeaway_delivery": bool(request.form.get(
                    "takeaway_delivery")),
    ...
    if "is_admin" in mongo.db.accounts.find_one(
        ...
        if request.method == "POST":
            ...
            submit_fishery_facilities = {
                ...
                "stock_size": int(request.form.get("stock_size")),
                "rods": request.form.get("rods"),
                "onsite_tackle_shop": bool(
                    request.form.get("onsite_tackle_shop")),
                "toilet": bool(request.form.get("toilet")),
                "shower": bool(request.form.get("shower")),
                "cafe": bool(request.form.get("cafe")),
                "fridge": bool(request.form.get("fridge")),
                ...
                "tuition": bool(request.form.get("tuition")),
                "drive_to_swim": bool(request.form.get("drive_to_swim")),
                "takeaway_delivery": bool(
                    request.form.get("takeaway_delivery")),
                "dogs_allowed": bool(request.form.get("dogs_allowed")),
                "parking": bool(request.form.get("parking"))
            }
            # update payment and booking details
...
def delete_fishery(fishery_id):
    ...
    if "is_admin" in mongo.db.accounts.find_one(
            {"_id": ObjectId(session["user"])}):
        # add disable to the contact document to
        # preserve report and catch history
        disable_fishery_contact = mongo.db.fisheries.contact.find_one(
            {"_id": ObjectId(fishery_id)})
        disable_fishery_contact["disabled"] = True
        mongo.db.fisheries.contact.update_one(
            {"_id": ObjectId(fishery_id)}, {"$set": disable_fishery_contact})
        # delete the rest
...
def reviews(fishery_id):
    ...
    reviews = []
    # loop through the reviews and for each
    # one retrieve the username for the account_id
    for doc in fishery_reviews:
        author_id = doc['account_id']
        username = mongo.db.accounts.find_one(
            {"_id": ObjectId(author_id)})['username']
        doc.update({"username": username})
        reviews.append(doc)
        reviews.sort(
            key=lambda review_date: review_date['date'], reverse=True)
    return render_template(
    ...
    if session["user"]:
        if request.method == "POST":
            ...
            flash('You have successfully added\
                the review', 'success')
            return redirect(
                url_for('reviews', fishery_id=review["fishery_id"]))

        fishery_contact = mongo.db.fisheries.contact.find_one(
            {"_id": ObjectId(fishery_id)})
        return render_template(
            "add_review.html", fishery_contact=fishery_contact,
            fishery_id=fishery_id)
...
def edit_review(review_id):
    ...
    if session["user"] == fishery_review["account_id"]:
        if request.method == "POST":
            ...
            return redirect(
                url_for('reviews', fishery_id=fishery_review["fishery_id"]))

        fishery_review = mongo.db.reviews.find_one(
...
def delete_review(review_id):
    fishery_review = mongo.db.reviews.find_one({"_id": ObjectId(review_id)})
    # Make sure the person executing this
    # function is the person that wrote the review
    # or an admin
    if session["user"] == fishery_review["account_id"] or session["is_admin"]:
        ...
        if session["user"] == fishery_review["account_id"]:
            return redirect(
                url_for('reviews', fishery_id=fishery_review["fishery_id"]))
        # If the delete was created by an admin
        # take the admin back to the moderation page
...
def report_review(review_id):
    ...
    mongo.db.reviews.update(
        {"_id": ObjectId(review_id)}, fishery_review)
    flash("Thank you. This review has been submitted to the website\
        administrators for consideration", 'info')
    return redirect(
        url_for('reviews', fishery_id=fishery_review["fishery_id"]))
...
def moderate_reviews():
    ...
    if session["is_admin"]:
        ...
        if len(fishery_reviews) == 0:
            return render_template(
                "moderation_reviews.html", fishery_reviews=fishery_reviews)
...
def reports(fishery_id):
    fishery_contact = mongo.db.fisheries.contact.find_one(
        {"_id": ObjectId(fishery_id)})
    fishery_reports = list(mongo.db.catch.reports.find(
        {"fishery_id": fishery_id}))
    fishery_catches = list(mongo.db.catch.fish.find(
        {"fishery_id": fishery_id}))
    reports = []
    # loop through the reports and for each one
    # retrieve the username for the account_id
        ...
        if request.method == "POST":
            ...
            report = {
                ...
                "date": datetime.strptime(
                    request.form.get("report_date"), '%d-%b-%Y'),
                "notes": request.form.get("report_notes")
            }
            mongo.db.catch.reports.insert_one(report)
            # When a report is added the user is
            # passed to the add_fish function
...
def edit_report(report_id):
    fishery_report = mongo.db.catch.reports.find_one(
        {"_id": ObjectId(report_id)})
    # Make sure the person executing this function
    # the person that created the report
    if session["user"] == fishery_report["account_id"]:
        if request.method == "POST":
            report = mongo.db.catch.reports.find_one(
                {"_id": ObjectId(report_id)})
            report_catches = list(mongo.db.catch.fish.find(
                {"report_id": report_id}))
            updated_report = {"$set": {
            ...
            mongo.db.catch.reports.update_one(
                {"_id": ObjectId(report_id)}, updated_report)
            if not report_catches:
                return redirect(
                    url_for('reports', fishery_id=report["fishery_id"]))

            else:
                for catch in report_catches:
                    # each catch list item has its
                    # own unique id name and label values
                    str_catch_id = str(catch["_id"])
                    marked_delete = bool(
                        request.form.get(f"{str_catch_id}delete"))
                    if marked_delete:
                        mongo.db.catch.fish.delete_one(
                            {"_id": ObjectId(str_catch_id)})

                    else:
                        # In carp fishing weight is measured
                        # in pounds and ounces
                        # The most efficient way to store
                        # this measure is just in ounces
                        weight_lbs = int(
                            request.form.get(f"{str_catch_id}weight_lbs")) * 16
                        weight_oz = int(
                            request.form.get(f"{str_catch_id}weight_oz"))
...
def delete_report(report_id):
    report = mongo.db.catch.reports.find_one(
        {"_id": ObjectId(report_id)})
    # Make sure the person executing this function
    # is the same person deleting it
    if session["user"] == report["account_id"]:
        mongo.db.catch.reports.delete_one(
            {"_id": ObjectId(report_id)})
...
def add_fish(report_id):
    report = mongo.db.catch.reports.find_one({"_id": ObjectId(report_id)})
    # Make sure the person executing this function
    # is the same person that created the report
    if session["user"] == report["account_id"]:
        if request.method == "POST":
            fishery_contact = mongo.db.fisheries.contact.find_one(
                {"_id": ObjectId(report["fishery_id"])})
            ...
            weight = weight_oz + weight_lbs
                ...
                "weight": weight,
                "date": datetime.strptime(
                    request.form.get("catch_date"), '%d-%b-%Y'),
                "time": datetime.strptime(
                    request.form.get("catch_time"), '%H:%M')
            }
        ...
        fishery_contact = mongo.db.fisheries.contact.find_one(
            {"_id": ObjectId(report["fishery_id"])})
        return render_template(
            "add_fish.html", fishery_contact=fishery_contact,
...
def contact():
    if request.method == "POST":
        ...
        flash('Your message has been sent.\
             Thank you for getting in touch', 'info')
